    ...
    @if (isset($category_id))
        ...
        @if ($filterAvailable == 'Yes')
            <div class="form-group">
                <label for="{{ $filter['filter_column'] }}">{{ __('Chọn ') }}{{ $filter['filter_name'] }}</label>
                <select name="{{ $filter['filter_column'] }}" id="{{ $filter['filter_column'] }}" class="form-control"
                    ...
                    @foreach ($filter['filter_value'] as $value)
                        <option value="{{ $value['filter_value'] }}"
                            @if (!empty($product[$filter['filter_column']]) && $value['filter_value'] == $product[$filter['filter_column']]) selected="" @endif>
